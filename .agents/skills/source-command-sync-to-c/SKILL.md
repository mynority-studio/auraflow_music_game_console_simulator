---
name: "source-command-sync-to-c"
description: "同步 TS 算法变更到 C 移植（ESP32 端）— 变更检测 → 翻译 → 编译 → 测试 → 黄金种子 → 文档同步"
---

# source-command-sync-to-c

Use this skill when the user asks to run the migrated source command `sync-to-c`.
...
C 项目路径：`/home/hsycc/Codex/auraflow_music_game_console/main/aura_radio/`
...
用 Docker 编译：
```bash
docker run --rm -v /home/hsycc/Codex/auraflow_music_game_console:/src -w /src gcc:12 \
...
跑全部 4 个测试套件：
```bash
docker run --rm -v /home/hsycc/Codex/auraflow_music_game_console:/src -w /src gcc:12 sh -c "
...
- Pipeline Rule 变更 → 同步 `.Codex/rules/aura_radio_v4_pipeline_rule.md`
