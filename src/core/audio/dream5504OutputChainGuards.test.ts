...
describe('Dream 5504 hardware MIDI output guards', () => {
    it('MidiScheduler no longer dispatches events to the browser synth fallback', () => {
        ...
        expect(source).toContain('this.notifyMidiEventListeners(ev);');
    });
    ...
    it('AudioEngine requires Dream5504MidiOutput before generated or uploaded playback', () => {
        ...
        const generated = source.slice(source.indexOf('public async playMusicGeneration'));
        const uploaded = source.slice(source.indexOf('public async playUploadedMidi'));
