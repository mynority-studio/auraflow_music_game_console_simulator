...
describe('instrumental/gestureExpression', () => {
  it('器配层把 lead sax 映射到 sax-breath-legato 手势计划', () => {
    const plan = gestureExpressionForProgram('lead', 66);
    expect(plan.kind).toBe('sax-breath-legato');
    expect(plan.family).toBe('sax');
    expect(plan.breathModel).toBe('reed-continuous');
    expect(plan.noteShape).toBe('legato-overlap');
    expect(plan.ccControllers).toEqual([SAX_CC.expression, SAX_CC.breath, SAX_CC.modulation]);
  });
  ...
  it('buildGestureExpressionByRole 跟随器配最终 roleProgram', () => {
    const byRole = buildGestureExpressionByRole(['lead', 'comp', 'bass'], { lead: 66, comp: 4, bass: 32 }, 'jazz');
  ...
  it('执行 sax 手势计划会同时改变连吹音符和生成 CC11/CC2/CC1', () => {
    const track: TrackIR = { role: 'lead', notes: [note(0, 120, 60), note(240, 960, 62)], program: 66 };
    const out = applyGestureExpressionToTrack(track, gestureExpressionForProgram('lead', 66), tb);
  ...
  it('端到端:render 消费器配下发的 sax 手势计划,生成 CC 和连吹 overlap', () => {
    const trace = traceGeneration({ seed: 1, styleHint: 'jazz', mood: 'build', targetDuration: 90 });
    expect(trace.lines.some((line) => line.includes('lead:sax GM66:sax-breath-legato'))).toBe(true);
