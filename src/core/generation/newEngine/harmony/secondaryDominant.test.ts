...
// 离调由 tonicizationPlanner 产出(degree-picker fallback 段上);planner-specific 标记 = tonicizationPlacement。
// POP/JAZZ/RNB tonMaxSong>0 会触发;LOFI/BLUES=0 不触发(prototype 自带 borrowedSource 不算 planner 离调)。
describe('harmony · 副属/离调 tonicizationPlanner (3.2)', () => {
  ...
  const tonChords = (plan: ReturnType<typeof mkPlan>) =>
    plan.chordTimeline.filter((c) => c.tonicizationPlacement !== undefined);
  const firstFiring = (style: string) => {
    for (let s = 0; s < 30; s++) { const p = mkPlan(style, s); if (tonChords(p).length > 0) return { plan: p, seed: s }; }
    return null;
  };

  it('★ POP/JAZZ/RNB 跨 seed 会触发 planner 离调(tonicizationPlacement)', () => {
    for (const style of ['pop', 'jazz', 'rnb']) {
  ...
  it('离调 V/X:根 = 临时主音上方五度,3 音落调外(chromatic 色彩)', () => {
    const f = firstFiring('jazz') ?? firstFiring('pop')!;
  ...
  it('离调 V/X 走 forcedScale(Mixolydian/Phrygian Dominant)→ chord-scale 非母调', () => {
    const f = firstFiring('jazz') ?? firstFiring('pop')!;
  ...
  it('确定性 + 排比不破:同 seed 两次一致 + verse1 ≡ verse2', () => {
    const f = firstFiring('jazz') ?? firstFiring('pop')!;
    const a = mkPlan(f.plan === null ? 'jazz' : 'jazz', f.seed);
    const sig = (p: typeof a) => p.chordTimeline.map((c) => `${c.rootPc}:${c.quality}@${c.startBeat}`).join('|');
    expect(sig(mkPlan('jazz', f.seed))).toBe(sig(mkPlan('jazz', f.seed)));
