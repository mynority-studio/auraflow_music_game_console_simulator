// ============================================================
// harmonic-coherence.ts — 跨 chord 债务簿 + 评分诊断(Universal)
// ============================================================
//
// 来源:melodygenerative/src/lib/harmonicCoherence.ts(2026-05-25 port)
//
// 核心哲学(per user music-theory rule):
//   "有张力,就要知道它为什么存在、期待什么、是否兑现"
//
// Universal "debt model":每个 chord 创建张力 → 生成 ResolutionObligation
// 声明:
//   - 张力是什么(tendency tones:3rd / b7 / b9 等)
//   - 期待什么 target(V/X 期待 X)
//   - 哪些音必须移到哪里(B → C / F → E for G7→C)
//   - target 必须稳定多长时间
//   - 未解决是否 OK(style-dependent)
//
// Style policies:5 style(POP / JAZZ / RNB / BLUES / LOFI)各有 strictness +
// stability beats + 容忍 unresolved color 标准。
//
// Phase 1(本文件):diagnostic only — 跑评分 + 列 issue。不 mutate 生成。
// 后续:UI display(显示评分)+ selective repair(低分 progression 重抽)。
//
// 独立 module:不 import 任何引擎 IR,plain CoherenceChordInput in / CoherenceReport out。
// ============================================================

// ─────────────────────────────────────────────────────────────────
// Plain data interface — caller adapter
// ─────────────────────────────────────────────────────────────────

/** Caller 传入的 chord 数据(plain object,跟 ChordDef IR 解耦) */
export interface CoherenceChordInput {
  rootMidi: number;
  bassMidi: number;
  notesMidi: number[];
  type: string;            // 'maj' / 'min' / 'maj7' / 'm7' / 'dom7' / etc.
  roman: string;           // 'I' / 'V' / 'V/vi' / 'subV/V' / 'bVII' / etc.
  duration: number;        // beats
  chordSymbol?: string;    // 'C' / 'G7' / 'Dm7' — UI 显示用
  effectiveFunc?: 'T' | 'S' | 'D';
  borrowedSource?: string; // 'secondary_dominant' / 'secondary_ii_v' / 'backdoor_dominant' / 'modal_interchange'
  borrowedFrom?: string;   // 'bII of vi (local ...)' 文本
  analysisKeyPc?: number;  // local tonal center pc(可选)
  localTonalCenterPc?: number;
  localRoman?: string;     // 'V' / 'I' 等局部 Roman
  mustResolve?: boolean;
}
...
export type ResolutionRole =
  ...
  | 'local_modal_color'
  | 'home_dominant'
  | 'dominant_evasion_to_modal_cadence'
  | 'blues_color';
...
function chordPcs(chord: CoherenceChordInput): Set<number> {
...
function isBluesColorChord(chord: CoherenceChordInput, globalKeyPc: number, policyStyle: string): boolean {
...
function makeDominantObligation(
  chord: CoherenceChordInput, index: number, targetPc: number,
  targetQuality: 'major' | 'minor', role: ResolutionRole, severity: 'hard' | 'medium',
  next: CoherenceChordInput | null, policy: HarmonicCoherencePolicy,
  ...
  const t = chord.type;
  if (t.includes('b9')) {
    tendencies.push({ fromPc: pc(rootPc + 1), toPc: targetFifth, direction: 'down', weight: 0.7, name: 'b9 → target 5th' });
  }
  if (t.includes('#9')) {
    tendencies.push({ fromPc: pc(rootPc + 3), toPc: targetRoot, direction: 'up', weight: 0.5, name: '#9 → target root' });
  }
  if (t.includes('b13')) {
    tendencies.push({ fromPc: pc(rootPc + 8), toPc: targetFifth, direction: 'down', weight: 0.6, name: 'b13 → target 5th' });
  }
...
function makeBackdoorObligation(
  chord: CoherenceChordInput, index: number, globalKeyPc: number,
  next: CoherenceChordInput | null, policy: HarmonicCoherencePolicy,
): ResolutionObligation {
  const rootPc = pc(chord.rootMidi);
  const third = pc(rootPc + 4);
 ...
 * V leading tone 故意不解决到 tonic,通过 bVII 实现 Aeolian modal cadence。
 */
function makeDominantEvasionObligation(
  vChord: CoherenceChordInput, vIndex: number,
  bviiChord: CoherenceChordInput, tonicChord: CoherenceChordInput, tonicIndex: number,
  globalKeyPc: number, policy: HarmonicCoherencePolicy,
): ResolutionObligation {
  ...
  return {
    ...
    targetPc: globalKeyPc,
    tendencies,
    targetStability: { minBeats: policy.targetStabilityBeats, requireStrongBeat: true, weight: 0.7 },
    allowPivot: true,
    severity: 'soft',
  };
}
...
function detectDominantEvasion(
  chords: CoherenceChordInput[], i: number, globalKeyPc: number,
): { bvii: CoherenceChordInput; tonic: CoherenceChordInput; tonicIndex: number } | null {
  const v = chords[i];
...
export function buildResolutionLedger(
  chords: CoherenceChordInput[], policy: HarmonicCoherencePolicy, globalKeyPc: number,
): ResolutionObligation[] {
...
export function scoreChordIdentity(chords: CoherenceChordInput[]): number {
  ...
  for (const chord of chords) {
    ...
    const t = chord.type;
    const has = (iv: number): boolean => pcs.has(pc(rootPc + iv));
    if (q === 'min' && (t === 'min' || t === 'm7' || t === 'm9' || t === 'm11')) {
      totalChecks++; if (has(3)) passedChecks++;
    }
...
export function scoreGuideTones(chords: CoherenceChordInput[], policy: HarmonicCoherencePolicy): number {
  if (chords.length === 0) return 1;
  let total = 0;
  let sumScore = 0;
  for (const chord of chords) {
...
export function scoreTendencyResolution(
  obligations: ResolutionObligation[], chords: CoherenceChordInput[], policy: HarmonicCoherencePolicy,
): number {
  ...
  for (const o of obligations) {
    if (o.targetChordIndex === null) {
      ...
      const sum = o.tendencies.reduce((s, t) => s + t.weight, 0);
      weightTotal += sum;
      weightHit += allowance * sum;
      continue;
    }
    const targetPcs = chordPcs(chords[o.targetChordIndex]!);
...
export function scoreBassMotion(
  obligations: ResolutionObligation[], chords: CoherenceChordInput[],
): number {
  if (obligations.length === 0) return 1;
  let total = 0;
  let sumScore = 0;
...
export function scoreLocalColorRoles(
  chords: CoherenceChordInput[], globalKeyPc: number, policy: HarmonicCoherencePolicy,
): number {
  ...
  for (const i of localColorIndices) {
    ...
    if (next && pc(next.rootMidi) === localCenterPc) role = 'neighbor_return';
             ...
             && next2 && pc(next2.rootMidi) === globalKeyPc) role = 'pivot_to_global';
    let s = 0.5;
...
export function collectIssues(
  chords: CoherenceChordInput[], obligations: ResolutionObligation[],
  policy: HarmonicCoherencePolicy, globalKeyPc: number,
): CoherenceIssue[] {
  ...
  for (let i = 0; i < chords.length; i++) {
    ...
    if (policy.style === 'JAZZ' || policy.guideToneStrictness > 0.7) {
      if (q === 'min' && chord.effectiveFunc === 'S' && !has(10)) {
        issues.push({ chordIndex: i, kind: 'missing_guide_tones', severity: 'mid',
          message: `${chord.chordSymbol ?? chord.roman}: minor predominant missing b7 (jazz ii needs 3+b7)` });
      }
      if (q === 'dom' && chord.effectiveFunc === 'D' && (!has(4) || !has(10))) {
        issues.push({ chordIndex: i, kind: 'missing_guide_tones', severity: 'high',
          message: `${chord.chordSymbol ?? chord.roman}: dominant missing 3 or b7` });
      }
    }
    if (chord.roman.startsWith('subV/') && pc(chord.bassMidi) !== pc(chord.rootMidi)) {
      issues.push({ chordIndex: i, kind: 'weak_subV_bass', severity: 'mid',
        message: `${chord.chordSymbol ?? chord.roman}: subV with non-root bass weakens half-step bass resolution` });
    }
    ...
    if (/^bII of /.test(bf)) {
      ...
      if (!isNeighborReturn && !isLocalPredominant && !isPivotToGlobal) {
        issues.push({ chordIndex: i, kind: 'local_bII_decorative_tail', severity: 'mid',
          message: `${chord.chordSymbol ?? chord.roman} (local bII): no clear role — not neighbor / pivot / predominant` });
      }
    }
  }
  if (chords.length > 0) {
    ...
    if (last.mustResolve) {
      issues.push({ chordIndex: chords.length - 1, kind: 'unresolved_final_dominant', severity: 'high',
        message: `Song ends on ${last.chordSymbol ?? last.roman} (mustResolve=true) without target` });
    }
  }
  for (const o of obligations) {
    if (o.targetChordIndex !== null) {
      ...
      if (target.duration < policy.targetStabilityBeats && next2?.mustResolve) {
        issues.push({ chordIndex: o.targetChordIndex, kind: 'target_not_stable', severity: 'mid',
          message: `Target ${target.chordSymbol ?? target.roman} only ${target.duration} beats before another resolution demand` });
      }
    }
  }
  for (const o of obligations) {
    if (o.severity === 'hard' && o.targetChordIndex === null && !policy.allowDelayedResolution) {
      ...
      issues.push({ chordIndex: o.sourceChordIndex, kind: 'unresolved_hard_obligation', severity: 'high',
        message: `${src.chordSymbol ?? src.roman} (${o.role}) didn't resolve to expected target pc=${o.targetPc}` });
    }
  }
  return issues;
}
...
export function evaluateHarmony(
  chords: CoherenceChordInput[], styleName: string, globalKeyPc: number,
): CoherenceReport {
