...
export function getChordVoicingAesthetics(chordType: string) {
    ...
    // dom7 route would otherwise win and mark interval 5 (the sus 4th)
...
function chordIdentityIntervals(chordType: string): Set<number> {
  ...
  if (chordType === '11') {
    result.add(10); // b7
    result.add(17 % 12); // 11 (= pc 5)
    // 11 chord traditionally OMITS 3 (m9 clash with 11), so 3 is NOT identity
    return result;
  }
...
export function classifyLickPolicyFamily(chordType: string): keyof typeof LICK_DEGREE_POLICY {
  ...
  // Sus + 7 = dominant_sus (7sus4, 9sus4, 7sus, 13sus4, etc.)
  if (/sus/.test(chordType) && /^(7|9|11|13)/.test(chordType)) return 'dominant_sus';
  ...
  // not 'maj' (handled above).
  if (/^m/.test(chordType)) return 'minor_add';
  // Dominant 7/9/11/13/alt — chord 3 is M3, chord 7 is b7.
  if (chordType === 'dom7' || /^(7|9|11|13)/.test(chordType)) return 'dominant';
  // Major triads + add9/6/9/sus2/sus4 (sus without 7) → major_add (4 stable
  // for sus, but otherwise major rules).
  return 'major_add';
}
...
export const JAZZ_ROOTLESS_VOICINGS: Record<string, number[]> = {
  ...
  // '11' chord — strict dom11 with 3 + b7 + 11. The natural 11 sits a
  // minor-9 above the 3rd (E↔F on G11), which IS a classic clash. Two
  // historical resolutions:
  //   (a) drop the 3 → effectively 9sus4 with a different label
  //   (b) keep the 3 and treat the 11 as #11 voiced an octave up
  // Per User strict identity rule "11 chord must have 3 unless it's
  // labeled sus", we keep 3 here. Callers wanting (a) should write
  // '9sus4' explicitly. Stage 2's POP_VOICINGS / RNB_VOICINGS already
  // applies sus-style voicing for that case.
  '11':        [4, 10, 14, 17],     // 3 b7 9 11 — strict dom11 identity
...
export const POP_VOICINGS: Record<string, number[]> = {
  // Note on '11': previously voiced [0, 5, 7, 10, 14] = root 4 5 b7 9
  // (sus interpretation, dropped 3 to dodge 11/3 m9 clash). Removed —
  // per the strict chord-identity rule, the dom11 spelling must keep 3
  // and callers wanting the sus interpretation should write '9sus4'
  // (which has its own canonical voicing). Falling through to the
  // CHORD_TYPES['11'] + chord-identity guard keeps 3 + b7 + 11.
};
...
export const RNB_VOICINGS: Record<string, number[]> = {
  ...
  // '11' — strict dom11 identity: 3 + b7 + 11. Previously voiced
  // [5, 10, 14, 17] (sus interpretation drop-3), but per the chord-
  // identity rule '11' must keep 3 and the 11. Callers wanting the
  // sus interpretation should write '9sus4' explicitly (entry above).
  '11':        [4, 10, 14, 17],       // 3 b7 9 11 — dom11 with 3 retained
};
