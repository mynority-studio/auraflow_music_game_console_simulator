// Export the full event-level curation candidate for A/B review.
...
const outputDir = resolve(process.argv[2] ?? 'tmp/video-replica/take-five-full-curation-v2');
const midiPath = resolve(outputDir, 'take-five-full-curation-v2.mid');
const logPath = resolve(outputDir, 'take-five-full-curation-v2.notes.json');
...
writeFileSync(logPath, `${JSON.stringify({
  schemaVersion: 1,
  status: 'full event-level A/B candidate; not the product baseline',
  source: score.source,
  evidence: {
